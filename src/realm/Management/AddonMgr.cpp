...
void AddonMgr::LoadFromDB()
{
    ...
    bool success = false;
    QueryResult* result = sCharSQL->Query(&success, loadClientAddons);
    if (!success)
    {
        LogError("AddonMgr", "Query failed: %s", loadClientAddons);
        return;
    }
    if (!result)
    {
    ...
    for (itr = mKnownAddons.begin(); itr != mKnownAddons.end(); ++itr)
    {
        if (itr->second->isNew)
        {
            ...
            ss << "INSERT INTO clientaddons (name, crc, banned, showinlist) VALUES(\""
                << sCharSQL->EscapeString(itr->second->name) << "\",\""
                << itr->second->crc << "\",\""
                << itr->second->banned << "\",\""
                << itr->second->showinlist << "\");";

            sCharSQL->Execute(ss.str().c_str());
        }
    }
}
